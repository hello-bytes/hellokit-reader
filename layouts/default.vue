<template>
    <div class="rss_container">
        ...
        <div v-show="showMiddlePanel" class="rss_middle_bar">
            ...
            <a :class="{feed_entry_active:isTodayPage}" class="middle_entry" href="/today">
            ...
            <a href="/read-later" :class="{feed_entry_active:isReadLater}" class="middle_entry" style="padding-left:22px;">
                <el-icon :size="18"><CollectionTag></CollectionTag></el-icon>
                <span>稍后阅读</span>
            </a>
            <a href="/already-read" :class="{feed_entry_active:isReaded}" class="middle_entry" style="padding-left:22px;">
            ...
            <a :class="{feed_entry_active:isMyFeed}" href="/all" class="middle_entry">
...
export default defineNuxtComponent({
    components: {
		RssLogo,SideBar,AddRss,User,RssLogoGray,CreateRssFolder,AllFeed,LaterRead,Recent,AllSubscribeFeed,AllFeedItem,Today,CreateFolderPopup,ArrowRight,ArrowDown,AllSubscribeFeedActive,Files,CollectionTag,ReadedIcon
	},
    ...
    async asyncData(params) {
        ...
        if (process.server) {
            ...
            }else if(url.indexOf("/today") >= 0){
                isTodayPage = true;
            }else if(url.indexOf("/all") >= 0){
                isMyFeed = true;
            }else if(url.indexOf("/read-later") >= 0){
                isReadLater = true;
            }else if(url.indexOf("/already-read") >= 0){
                isReaded = true;
            }
    ...
    methods : {
        ...
        onSelectFolder(folder){
            ...
            if (Date.now() - this.currentExpandTick > 700) {
                window.location.href = "/folder/" + folder.folder_id + "/1" + ".html";    
            }
        }
    }
})
